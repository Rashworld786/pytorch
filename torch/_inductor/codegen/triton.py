...
class TritonIndexExprPrinter(TritonPrinter):
    ...
    def _print_Symbol(self, expr: sympy.Expr) -> str:
        result = super()._print_Symbol(expr)
        if expr.is_integer:
...
class TritonKernelOverrides(TritonOverrides):
    ...
    def index_expr(cls, expr, dtype):
        ...
        if dtype not in (torch.int32, torch.int64):
            ...
            if actual_dtype != expr_dtype:
                var = V.kernel.cse.generate(
                    V.kernel.compute,
                    cls.to_dtype(var, expr_dtype),
                    dtype=expr_dtype,
                    shape=var.shape,
                )

        if (
            requested_dtype in (torch.int32, torch.int64)
            and var.dtype != requested_dtype
        ):
            var = V.kernel.cse.generate(
                V.kernel.compute,
                cls.to_dtype(var, requested_dtype),
...
class TritonKernel(SIMDKernel[TritonCSEVariable]):
    ...
    def indexing(
        ...
        *,
        ...
        index_str_dtype = (
            triton_type(expr_dtype)
            if expr_dtype in (torch.int32, torch.int64)
            else "tl.int32"
        )
